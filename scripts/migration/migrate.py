...
@dataclass
class PhaseStats:
    """Estatísticas de execução de uma fase de migração."""
    lidos: int = 0
    inseridos: int = 0
    atualizados: int = 0
    pulados: int = 0
    ...
    def log_summary(self, phase_id: str) -> None:
        log.info(
            f"Fase {phase_id} stats: lidos={self.lidos} inseridos={self.inseridos} "
            f"atualizados={self.atualizados} pulados={self.pulados} "
            f"erros={self.erros} rejeitos={self.rejeitos}"
        )
...
def normalize_agencia(agencia: str) -> str:
    """Normaliza string de agência: remove pontuação, espaços, zero-padding."""
    if not agencia:
        return ""
    cleaned = re.sub(r"[\s.\-/]", "", agencia.strip())
...
class BasePhase:
    ...
    def _insert_endereco(
        ...
        # Defensivo: aceita int ou str
        try:
            if isinstance(numero, int):
                num_int = numero
            else:
                num_int = int(re.sub(r"\D", "", str(numero) if numero else "0") or "0")
        except (ValueError, TypeError):
            num_int = 0
    ...
    def _insert_profissoes_batch(self, prof_rows: List[tuple]) -> None:
        """
        Insere profissões e vincula com pessoas.
        prof_rows = [(pessoa_id, nome_profissao, renda), ...]
                  ou [(pessoa_id, nome_profissao, renda, empresa), ...]
        """
        if not prof_rows:
            return

        # 1. Coleta nomes únicos de profissões
        unique_names = list(set(r[1] for r in prof_rows))

        # 2. Insere as que não existem
        for nome in unique_names:
            self.writer.execute(
                "INSERT INTO profissoes (nome, descricao, ativo) VALUES (%s, %s, TRUE) ON CONFLICT DO NOTHING",
                (nome, nome)
            )

        # 3. Busca IDs de todas
        placeholders = ",".join(["%s"] * len(unique_names))
        rows = self.writer.fetch_all(
            f"SELECT id, nome FROM profissoes WHERE nome IN ({placeholders})",
            tuple(unique_names)
        )
        prof_id_map = {r[1]: r[0] for r in rows}

        # 4. Insere vínculos em pessoas_profissoes
        vinculos = []
        for row in prof_rows:
            pessoa_id, nome, renda = row[0], row[1], row[2]
            empresa = row[3] if len(row) > 3 else None
            prof_id = prof_id_map.get(nome)
            if prof_id:
                vinculos.append((
                    pessoa_id, prof_id,
                    empresa[:100] if empresa else None,
                    str(round(renda, 2)) if renda > 0 else None,
                ))
        ...
        if vinculos:
            with self.writer.conn.cursor() as cur:
                psycopg2.extras.execute_values(
                    cur,
                    """INSERT INTO pessoas_profissoes
                       (id_pessoa, id_profissao, empresa, renda, ativo)
    ...
    def resolve_or_create_pessoa(
        ...
        nome: str,
        data_nascimento: Optional[date],
        tipo_pessoa_id: Optional[int],
        tipo_pessoa_str: Optional[str],
        fisica_juridica: str,
        estado_civil_id: Optional[int],
        renda: Optional[float],
        nome_pai: Optional[str],
        nome_mae: Optional[str],
        observacoes: Optional[str],
        nacionalidade_id: Optional[int],
        naturalidade_id: Optional[int],
        namespace: str,
        old_id: int,
    ) -> Optional[int]:
        try:
            # Priority 1: CPF
            cpf_clean = clean_doc(cpf) if cpf else ""
            cnpj_clean = clean_doc(cnpj) if cnpj else ""
            if cpf_clean and cpf_clean not in ("0", "00") and len(cpf_clean) >= 8:
                res = self.writer.fetch_one(
                    "SELECT id_pessoa FROM pessoas_documentos WHERE id_tipo_documento = %s AND numero_documento = %s LIMIT 1",
                    (cfg.DEFAULT_TIPO_DOCUMENTO_CPF_ID, cpf_clean))
                if res:
                    pessoa_id = res[0]
                    log.info(f"Found pessoa by CPF {cpf}")
                    return self._update_add_pessoa_details(pessoa_id, cpf, cnpj, rg, nome, data_nascimento, tipo_pessoa_id, tipo_pessoa_str, fisica_juridica, estado_civil_id, renda, nome_pai, nome_mae, observacoes, nacionalidade_id, naturalidade_id, namespace, old_id)
            # Priority 1: CNPJ
            if cnpj_clean and cnpj_clean not in ("0", "00") and len(cnpj_clean) >= 10:
                res = self.writer.fetch_one(
                    "SELECT id_pessoa FROM pessoas_documentos WHERE id_tipo_documento = %s AND numero_documento = %s LIMIT 1",
                    (cfg.DEFAULT_TIPO_DOCUMENTO_CNPJ_ID, cnpj_clean))
                if res:
                    pessoa_id = res[0]
                    log.info(f"Found pessoa by CNPJ {cnpj}")
                    return self._update_add_pessoa_details(pessoa_id, cpf, cnpj, rg, nome, data_nascimento, tipo_pessoa_id, tipo_pessoa_str, fisica_juridica, estado_civil_id, renda, nome_pai, nome_mae, observacoes, nacionalidade_id, naturalidade_id, namespace, old_id)
            # Priority 2: RG + nome + data_nascimento
            if rg and data_nascimento and data_nascimento != date(1900, 1, 1):
                rg_clean = clean_doc(rg)
                if rg_clean and rg_clean not in ("0", "00"):
                    res = self.writer.fetch_one(
                        """SELECT pd.id_pessoa FROM pessoas_documentos pd
                           JOIN pessoas p ON p.idpessoa = pd.id_pessoa
                           WHERE pd.id_tipo_documento = %s AND pd.numero_documento = %s
                             AND UPPER(p.nome) = UPPER(%s) AND p.data_nascimento = %s
                           LIMIT 1""",
                        (cfg.DEFAULT_TIPO_DOCUMENTO_RG_ID, rg_clean, nome, data_nascimento))
                if res:
                    pessoa_id = res[0]
                    log.info(f"Found pessoa by RG {rg}")
                    return self._update_add_pessoa_details(pessoa_id, cpf, cnpj, rg, nome, data_nascimento, tipo_pessoa_id, tipo_pessoa_str, fisica_juridica, estado_civil_id, renda, nome_pai, nome_mae, observacoes, nacionalidade_id, naturalidade_id, namespace, old_id)
            # Priority 3: nome + data_nascimento (low confidence)
            if data_nascimento and data_nascimento != date(1900, 1, 1):
                res = self.writer.fetch_one(
                    "SELECT idpessoa FROM pessoas WHERE UPPER(nome) = UPPER(%s) AND data_nascimento = %s LIMIT 1",
                    (nome, data_nascimento))
                if res:
                    pessoa_id = res[0]
                    log.info(f"Found pessoa by nome and data_nascimento")
                    return self._update_add_pessoa_details(pessoa_id, cpf, cnpj, rg, nome, data_nascimento, tipo_pessoa_id, tipo_pessoa_str, fisica_juridica, estado_civil_id, renda, nome_pai, nome_mae, observacoes, nacionalidade_id, naturalidade_id, namespace, old_id)
            # No match: create new
            pessoa_id = self._insert_pessoa(
                nome=nome,
                tipo_pessoa=tipo_pessoa_id or 4,
            ...
            log.info(f"Created new pessoa id {pessoa_id}")
            if cpf:
                self._insert_documento(pessoa_id, clean_doc(cpf), cfg.DEFAULT_TIPO_DOCUMENTO_CPF_ID)
            if cnpj:
                self._insert_documento(pessoa_id, clean_doc(cnpj), cfg.DEFAULT_TIPO_DOCUMENTO_CNPJ_ID)
            if rg:
                self._insert_documento(pessoa_id, clean_doc(rg), cfg.DEFAULT_TIPO_DOCUMENTO_RG_ID)
            if tipo_pessoa_id:
                self.writer.execute(
                    "INSERT INTO pessoas_tipos (id_pessoa, id_tipo_pessoa, data_inicio, ativo) VALUES (%s, %s, CURRENT_DATE, true) ON CONFLICT DO NOTHING",
                    (pessoa_id, tipo_pessoa_id))
            self.state.set_mapping(namespace, str(old_id), pessoa_id)
            return pessoa_id
        except Exception as e:
            log.warning(f"Error resolving or creating pessoa: {e}")
            return None

    def _update_add_pessoa_details(
        self,
        pessoa_id: int,
        cpf: Optional[str],
        cnpj: Optional[str],
        ...
        try:
            # Update NULL fields only (never overwrite nome)
            updates = {
                'data_nascimento': data_nascimento,
                'estado_civil_id': estado_civil_id,
                'renda': str(round(renda, 2)) if renda and renda > 0 else None,
                'nome_pai': nome_pai,
                'nome_mae': nome_mae,
                'nacionalidade_id': nacionalidade_id,
                'naturalidade_id': naturalidade_id,
            }
            for col, val in updates.items():
                if val is not None:
                    self.writer.execute(
                        f"UPDATE pessoas SET {col} = %s WHERE idpessoa = %s AND {col} IS NULL",
                        (val, pessoa_id))
            # Add tipo_pessoa if missing
            if tipo_pessoa_id:
                res = self.writer.fetch_one(
                    "SELECT 1 FROM pessoas_tipos WHERE id_pessoa = %s AND id_tipo_pessoa = %s",
                    (pessoa_id, tipo_pessoa_id))
                if not res:
                    self.writer.execute(
                        "INSERT INTO pessoas_tipos (id_pessoa, id_tipo_pessoa, data_inicio, ativo) VALUES (%s, %s, CURRENT_DATE, true) ON CONFLICT DO NOTHING",
                        (pessoa_id, tipo_pessoa_id))
            # Add documents if missing (order: pessoa_id, numero, tipo_id)
            if cpf:
                self._insert_documento(pessoa_id, clean_doc(cpf), cfg.DEFAULT_TIPO_DOCUMENTO_CPF_ID)
            if cnpj:
                self._insert_documento(pessoa_id, clean_doc(cnpj), cfg.DEFAULT_TIPO_DOCUMENTO_CNPJ_ID)
            ...
            log.warning(f"Error updating pessoa details: {e}")
            return pessoa_id

    def _write_reject_csv(self, filename: str, fieldnames: List[str], rows: List[Dict[str, Any]]) -> None:
        """
        Writes rejected records to CSV file for audit trail.

        Args:
            filename: Output CSV file path (e.g., 'rejeitos_fase_05.csv')
            fieldnames: List of column names
            rows: List of dicts with rejected record data
        """
        if not rows:
            return
        try:
            ...
            log.info(f"Wrote {len(rows)} rejected records to {filename}")
        except Exception as e:
            log.error(f"Error writing reject CSV {filename}: {e}")


# ---------------------------------------------------------------------------
# Validação de Estado Civil (Correction 6 — análise, não modifica dados)
# ---------------------------------------------------------------------------

def validate_estado_civil_distribution(parser: MySQLDumpParser) -> None:
    """
    Analisa distribuição de estadocivil em locfiadores cruzando com presença de cônjuge.
    Resultado é logado — não altera dados.
    """
    dist: Dict[int, Dict[str, int]] = {}
    for row in parser.iter_table("locfiadores"):
        ec = safe_int(row.get("estadocivil"))
        has_conj = bool(safe_str(row.get("nomeconj")))
        if ec not in dist:
            dist[ec] = {"com_conjuge": 0, "sem_conjuge": 0}
        if has_conj:
            dist[ec]["com_conjuge"] += 1
        else:
            dist[ec]["sem_conjuge"] += 1

    log.info("=== VALIDAÇÃO ESTADO CIVIL (locfiadores) ===")
    for ec_val in sorted(dist.keys()):
        d = dist[ec_val]
        new_id = cfg.ESTADO_CIVIL_MAP.get(ec_val, "NAO MAPEADO")
        total = d["com_conjuge"] + d["sem_conjuge"]
        pct_conj = 100 * d["com_conjuge"] / total if total > 0 else 0
        log.info(
            f"  estadocivil={ec_val} -> novo id={new_id}: "
            f"{total} registros, {pct_conj:.1f}% com conjuge "
            f"(com={d['com_conjuge']} sem={d['sem_conjuge']})"
        )

    # Heuristic: if estadocivil=1 has >50% com_conjuge, mapping may be inverted
    ec1 = dist.get(1, {})
    total_ec1 = ec1.get("com_conjuge", 0) + ec1.get("sem_conjuge", 0)
    if total_ec1 > 0:
        pct_ec1_conj = 100 * ec1["com_conjuge"] / total_ec1
        if pct_ec1_conj > 50 and cfg.ESTADO_CIVIL_MAP.get(1) == 3:  # 3=solteiro
            log.warning(
                f"ATENCAO: estadocivil=1 tem {pct_ec1_conj:.1f}% com conjuge, "
                f"mas ESTADO_CIVIL_MAP[1]=3 (Solteiro). "
                f"Verificar se mapeamento esta invertido (1=Casado?)!"
            )
    log.info("=== FIM VALIDAÇÃO ESTADO CIVIL ===")
...
class Phase00Validacao(BasePhase):
    ...
    def run(self) -> int:
        ...
        # Seed formas_retirada (tabela vazia no banco novo)
        fr_count = self.writer.count("formas_retirada")
        if fr_count == 0:
            log.info("  Populando formas_retirada...")
            for fr_id, forma, descr in cfg.FORMAS_RETIRADA_SEED:
                self.writer.execute(
                    "INSERT INTO formas_retirada (id, forma, descricao) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING",
                    (fr_id, forma, descr)
                )
            self.writer.commit()
            log.info(f"  formas_retirada: {len(cfg.FORMAS_RETIRADA_SEED)} registros inseridos.")
        ...
        # Correction 6: Validar distribuição de estado civil (análise, não modifica dados)
        log.info("Iniciando validação de distribuição de estado civil...")
        validate_estado_civil_distribution(self.parser)
...
class Phase08Locadores(BasePhase):
    ...
    def run(self) -> int:
        ...
        for chunk_start in range(0, len(pending), self.CHUNK_SIZE):
            ...
            # Pre-resolve naturalidade IDs for this chunk
            nat_names = set()
            for row in chunk:
                nat = safe_str(row.get("naturalidade")) or ""
                if nat:
                    nat_names.add(nat[:100])

            nat_id_map = {}
            if nat_names:
                for nat_name in nat_names:
                    existing = self.writer.fetch_one(
                        "SELECT id FROM naturalidades WHERE UPPER(nome) = UPPER(%s) LIMIT 1",
                        (nat_name,)
                    )
                    if existing:
                        nat_id_map[nat_name.upper()] = existing[0]
                    else:
                        row_r = self.writer.execute_returning(
                            "INSERT INTO naturalidades (nome) VALUES (%s) RETURNING id",
                            (nat_name,)
                        )
                        if row_r:
                            nat_id_map[nat_name.upper()] = row_r[0]

            # --- Step 1: Resolve or create pessoas row-by-row ---
            new_ids = []
            for row in chunk:
                # Clean identifiers
                cpf_raw = row.get("cpf")
                cpf = safe_str(cpf_raw)
                if cpf:
                    cpf = cpf.strip()

                cnpj_raw = row.get("cnpj")
                cnpj = safe_str(cnpj_raw)
                if cnpj:
                    cnpj = cnpj.strip()
                ...
                data_nascimento = safe_date(row.get("nascto")) or date(1900, 1, 1)

                tipo_pessoa_old = safe_int(row.get("tipopessoa"))
                fisica_juridica = cfg.TIPO_PESSOA_MAP.get(tipo_pessoa_old, "fisica")

                estado_civil_old = safe_int(row.get("estadocivil"))
                estado_civil_id = cfg.ESTADO_CIVIL_MAP.get(estado_civil_old)

                renda = safe_float(row.get("renda"))

                obs = safe_str(row.get("obs")) or ""

                naturalidade_str = safe_str(row.get("naturalidade")) or ""
                naturalidade_id = nat_id_map.get(naturalidade_str.upper()) if naturalidade_str else None

                person_id = self.resolve_or_create_pessoa(
                    cpf=cpf,
                    cnpj=cnpj,
                    rg=rg,
                    nome=nome,
                    data_nascimento=data_nascimento,
                    tipo_pessoa_id=cfg.TIPO_PESSOA_LOCADOR_ID,
                    tipo_pessoa_str=cfg.TIPO_PESSOA_LOCADOR_ID,
                    fisica_juridica=fisica_juridica,
                    estado_civil_id=estado_civil_id,
                    renda=renda,
                    nome_pai=None,
                    ...
                    naturalidade_id=naturalidade_id,
                    namespace="loclocador",
                    old_id=safe_int(row.get("codigo")),
                )
                new_ids.append(person_id)

            if not new_ids:
                log.error(f"Fase 08: resolve_or_create_pessoa retornou 0 ids para {len(chunk)} locadores")
            ...
            for i, row in enumerate(chunk):
                ...
                for tel_field in ("telefone", "celular", "comercial", "fax", "outros"):
                ...
                if conta_cod and banco_old:
                    banco_new_id = self.state.get_new_id("banco", banco_old)
                    agencia_cod_norm = normalize_agencia(agencia_cod)
                    agencia_new_id = (
                        self.state.get_new_id("agencia", f"{banco_old}_{agencia_cod_norm}") or
                        self.state.get_new_id("agencia", f"{banco_old}_{agencia_cod}")
                    )
            ...
            if tels_data:
                ...
                # Build tel (numero, tipo_id) -> id map
                unique_tel_keys = {(numero, tipo_id) for _, numero, tipo_id in tels_data}
                tel_id_map: Dict[tuple, int] = {}
                if unique_tel_keys:
                    for numero, tipo_id in unique_tel_keys:
                        row_t = self.writer.fetch_one(
                            "SELECT id FROM telefones WHERE numero = %s AND id_tipo = %s",
                            (numero, tipo_id)
                        )
                        if row_t:
                            tel_id_map[(numero, tipo_id)] = row_t[0]
                junctions_tel = []
                for pessoa_id, numero, tipo_id in tels_data:
                    tel_id = tel_id_map.get((numero, tipo_id))
                    if tel_id:
            ...
            # ====== NOVO v3: INSERT INTO pessoas_locadores ======
            locadores_rows: List[tuple] = []
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                forma_ret_old = safe_int(row.get("formaretirada"))
                forma_ret_id = cfg.FORMA_RETIRADA_MYSQL_MAP.get(forma_ret_old)
                dependentes = safe_int(row.get("dependentes"))
                dia_ret = safe_int(row.get("diaretirada")) or None
                cobrar_cpmf = bool(safe_int(row.get("cobracpmf")))
                situacao_loc = safe_int(row.get("situacao"))
                cod_contabil = safe_int(row.get("codcontabil")) or None
                etiqueta = bool(safe_int(row.get("etiqueta")))
                cobrar_tarifa = bool(safe_int(row.get("cobratarifarec")))
                data_fech = safe_date(row.get("datafechamento"))
                carencia = safe_int(row.get("carencia"))
                multa_itau = bool(safe_int(row.get("multaitau")))
                mora_diaria = bool(safe_int(row.get("moradiaria")))
                protesto = safe_int(row.get("protesto"))
                dias_prot = safe_int(row.get("diasprot"))
                nao_gerar_jud = bool(safe_int(row.get("naogerarjudicial")))
                end_cobranca = bool(safe_int(row.get("endcobranca")))
                condo_conta = bool(safe_int(row.get("condoconta")))
                ext_email_loc = bool(safe_int(row.get("ext_email")))
                locadores_rows.append((
                    pessoa_id, forma_ret_id, dependentes, dia_ret,
                    cobrar_cpmf, situacao_loc, cod_contabil, etiqueta,
                    cobrar_tarifa, data_fech, carencia, multa_itau,
                    mora_diaria, protesto, dias_prot, nao_gerar_jud,
                    end_cobranca, condo_conta, ext_email_loc,
                ))
            if locadores_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO pessoas_locadores
                           (id_pessoa, id_forma_retirada, dependentes, dia_retirada,
                            cobrar_cpmf, situacao, codigo_contabil, etiqueta,
                            cobrar_tarifa_rec, data_fechamento, carencia, multa_itau,
                            mora_diaria, protesto, dias_protesto, nao_gerar_judicial,
                            endereco_cobranca, condominio_conta, ext_email)
                           VALUES %s ON CONFLICT DO NOTHING""",
                        locadores_rows,
                        template="(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    )

            # ====== NOVO v3.1: INSERT INTO pessoas_tipos (locador=4) ======
            tipos_rows = [(pid, cfg.TIPO_PESSOA_LOCADOR_ID) for pid in new_ids if pid]
            if tipos_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO pessoas_tipos (id_pessoa, id_tipo_pessoa, data_inicio, ativo)
                           VALUES %s ON CONFLICT DO NOTHING""",
                        tipos_rows,
                        template="(%s, %s, CURRENT_DATE, true)"
                    )

            # ====== NOVO v3: INSERT INTO profissoes + pessoas_profissoes ======
            prof_rows: List[tuple] = []
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                profissao_str = safe_str(row.get("profissao"))
                if profissao_str:
                    prof_rows.append((pessoa_id, profissao_str[:100], safe_float(row.get("renda"))))
            if prof_rows:
                self._insert_profissoes_batch(prof_rows)

            # ====== NOVO v3: INSERT INTO chaves_pix ======
            pix_rows: List[tuple] = []
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                tipo_pix_old = safe_int(row.get("tipoChavePix"))
                chave_pix = safe_str(row.get("ChavePix"))
                if chave_pix and tipo_pix_old > 0:
                    tipo_pix_new = cfg.TIPO_CHAVE_PIX_MAP.get(tipo_pix_old)
                    if tipo_pix_new:
                        pix_rows.append((pessoa_id, tipo_pix_new, chave_pix[:100]))
            if pix_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO chaves_pix
                           (id_pessoa, id_tipo_chave, chave_pix, principal, ativo)
                           VALUES %s ON CONFLICT DO NOTHING""",
                        pix_rows,
                        template="(%s, %s, %s, TRUE, TRUE)"
                    )
...
class Phase09Fiadores(BasePhase):
    ...
    def run(self) -> int:
        ...
        for chunk_start in range(0, len(pending), self.CHUNK_SIZE):
            ...
            # Pre-resolve nacionalidade IDs for this chunk
            nacion_names = set()
            for row in chunk:
                nacion = safe_str(row.get("nacion")) or ""
                if nacion:
                    nacion_names.add(nacion[:100])

            nacion_id_map = {}
            if nacion_names:
                for nacion_name in nacion_names:
                    if nacion_name.upper().startswith("BRASIL"):
                        nacion_id_map[nacion_name.upper()] = cfg.DEFAULT_NACIONALIDADE_ID
                    else:
                        existing = self.writer.fetch_one(
                            "SELECT id FROM nacionalidades WHERE UPPER(nome) = UPPER(%s) LIMIT 1",
                            (nacion_name,)
                        )
                        if existing:
                            nacion_id_map[nacion_name.upper()] = existing[0]
                        else:
                            row_r = self.writer.execute_returning(
                                "INSERT INTO nacionalidades (nome) VALUES (%s) RETURNING id",
                                (nacion_name,)
                            )
                            if row_r:
                                nacion_id_map[nacion_name.upper()] = row_r[0]
            ...
            for row in chunk:
                old_id = safe_int(row.get("codigo"))
                nome = safe_str(row.get("nome")) or "SEM NOME"
                nascto = safe_date(row.get("dtnasc")) or date(1900, 1, 1)
                ...
                nacion_str = safe_str(row.get("nacion")) or ""
                nacion_id = nacion_id_map.get(nacion_str.upper()) if nacion_str else None
                cpf = safe_str(row.get("cpf")) or ""
                rg = safe_str(row.get("rg")) or ""

                pessoa_id = self.resolve_or_create_pessoa(
                    cpf=cpf, cnpj="", rg=rg,
                    nome=nome[:100],
                    data_nascimento=nascto,
                    tipo_pessoa_id=cfg.TIPO_PESSOA_FIADOR_ID,
                    tipo_pessoa_str=cfg.TIPO_PESSOA_FIADOR_ID,
                    fisica_juridica="fisica",
                    estado_civil_id=estado_civil_id,
                    renda=renda,
                    nome_pai=pai[:100] if pai else None,
                    nome_mae=mae[:100] if mae else None,
                    observacoes=obs,
                    nacionalidade_id=nacion_id,
                    naturalidade_id=None,
                    namespace="locfiador",
                    old_id=old_id,
                )
                new_ids.append(pessoa_id)

            if not any(new_ids):
                log.error(f"Fase 09: resolve_or_create retornou 0 pessoas para {len(chunk)} fiadores")
                self.writer.rollback()
                continue
            ...
            # Batch telefones + junctions (compound key: numero+tipo)
            if tels_data:
                unique_tels = {}
                for _, num, tipo in tels_data:
                    unique_tels[(num, tipo)] = (tipo, num)
                ...
                # Build compound key map
                tel_id_map: Dict[tuple, int] = {}
                for (num, tipo) in unique_tels.keys():
                    row_t = self.writer.fetch_one(
                        "SELECT id FROM telefones WHERE numero = %s AND id_tipo = %s",
                        (num, tipo)
                    )
                    if row_t:
                        tel_id_map[(num, tipo)] = row_t[0]
                junctions_tel = []
                for pessoa_id, numero, tipo_id in tels_data:
                    tel_id = tel_id_map.get((numero, tipo_id))
                    if tel_id:
            ...
            # ====== NOVO v3: INSERT INTO pessoas_fiadores ======
            fiadores_rows: List[tuple] = []
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                motivo = safe_str(row.get("motivofianca"))
                ja_foi = bool(safe_int(row.get("jasaofiadores")))
                conj_trab = bool(safe_int(row.get("conjtrabalha")))
                outros = safe_str(row.get("outros"))
                fiadores_rows.append((
                    pessoa_id, motivo, ja_foi, conj_trab, outros,
                ))
            if fiadores_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO pessoas_fiadores
                           (id_pessoa, motivo_fianca, ja_foi_fiador, conjuge_trabalha, outros)
                           VALUES %s ON CONFLICT DO NOTHING""",
                        fiadores_rows,
                        template="(%s, %s, %s, %s, %s)"
                    )

            # ====== NOVO v3.1: INSERT INTO pessoas_tipos (fiador=1) ======
            tipos_rows = [(pid, cfg.TIPO_PESSOA_FIADOR_ID) for pid in new_ids if pid]
            if tipos_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
            ...
            for i, row in enumerate(chunk):
                ...
                if not pessoa_id:
                    continue
                atividade = safe_str(row.get("atividade"))
                empresa = safe_str(row.get("empresaf"))
                if atividade:
                    renda = safe_float(row.get("renda"))
                    prof_rows.append((pessoa_id, atividade[:100], renda, empresa))
            if prof_rows:
                self._insert_profissoes_batch(prof_rows)

            # ====== v3.2: CÔNJUGES — cria Pessoa para cada cônjuge do fiador ======
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                ...
                try:
                    ...
                    conjtrabalha_val = bool(safe_int(row.get("conjtrabalha") or row.get("conjtrabalhaf") or 0))
                    ...
                    # Resolve nacionconj to id
                    nacionconj_id = None
                    if nacionconj:
                        if nacionconj.lower().startswith("brasil"):
                            nacionconj_id = cfg.DEFAULT_NACIONALIDADE_ID
                        else:
                            nac_row = self.writer.fetch_one(
                                "SELECT id FROM nacionalidades WHERE UPPER(nome) = UPPER(%s) LIMIT 1",
                                (nacionconj,))
                            if nac_row:
                                nacionconj_id = nac_row[0]
                            else:
                                nac_row = self.writer.execute_returning(
                                    "INSERT INTO nacionalidades (nome) VALUES (%s) RETURNING id",
                                    (nacionconj,))
                                if nac_row:
                                    nacionconj_id = nac_row[0]

                    conj_id = self.resolve_or_create_pessoa(
                        cpf=cpfconj, cnpj="", rg=rgconj,
                        nome=nomeconj[:100],
                        data_nascimento=dtnascconj,
                        tipo_pessoa_id=cfg.TIPO_PESSOA_FIADOR_ID,
                        tipo_pessoa_str=cfg.TIPO_PESSOA_FIADOR_ID,
                        fisica_juridica="fisica",
                        estado_civil_id=None,
                        renda=rendaconj if rendaconj > 0 else None,
                        nome_pai=conjpaif[:100] if conjpaif else None,
                        nome_mae=conjmaef[:100] if conjmaef else None,
                        observacoes=obs,
                        nacionalidade_id=nacionconj_id,
                        naturalidade_id=None,
                        namespace="conj_fiador",
                        old_id=safe_int(row.get("codigo")),
                    )
                    if not conj_id:
                        continue
                    ...
                    if atividadeconj:
                        self._insert_profissoes_batch([(conj_id, atividadeconj[:100], rendaconj, conjempresaf)])

                    # Persist admissaoconj as data_admissao in pessoas_profissoes
                    if admissaoconj and atividadeconj:
                        self.writer.execute(
                            """UPDATE pessoas_profissoes pp
                               SET data_admissao = %s
                               FROM profissoes pr
                               WHERE pp.id_pessoa = %s AND pp.id_profissao = pr.id
                                 AND UPPER(pr.nome) = UPPER(%s) AND pp.data_admissao IS NULL""",
                            (admissaoconj, conj_id, atividadeconj[:100])
                        )

                    # Telefone do cônjuge
                    tel = re.sub(r"\s+", "", conjtelemp)[:25]
                    if tel:
                        self._insert_telefone(conj_id, tel, cfg.TIPO_TELEFONE_COMERCIAL_ID)

                    # Vincula cônjuge no registro do fiador
                    self.writer.execute(
                        "UPDATE pessoas_fiadores SET id_conjuge = %s, conjuge_trabalha = %s WHERE id_pessoa = %s",
                        (conj_id, conjtrabalha_val, pessoa_id)
                    )
                    log.debug(f"Cônjuge '{nomeconj}' -> pessoa {conj_id} (fiador {pessoa_id})")
                except Exception as e:
                    log.warning(f"Erro cônjuge fiador pessoa {pessoa_id}: {e}")
        ...
        for chunk_start in range(0, len(pending), self.CHUNK_SIZE):
            ...
            for row in chunk:
                ...
                pessoa_id = self.resolve_or_create_pessoa(
                    ...
                    nome=nome[:100],
                    data_nascimento=nascto,
                    tipo_pessoa_id=cfg.TIPO_PESSOA_CONTRATANTE_ID,
                    tipo_pessoa_str=cfg.TIPO_PESSOA_CONTRATANTE_ID,
                    fisica_juridica="fisica",
                    estado_civil_id=estado_civil_id,
                    renda=renda,
                    nome_pai=pai[:100] if pai else None,
                    ...
                    nacionalidade_id=nacion_id,
                    naturalidade_id=None,
                    namespace="loccontratante",
                    old_id=old_id,
                )
                new_ids.append(pessoa_id)

            if not any(new_ids):
                log.error(f"Fase 10: resolve_or_create retornou 0 pessoas para {len(chunk)} contratantes")
                self.writer.rollback()
                continue
            ...
            # ====== NOVO v3: INSERT INTO pessoas_contratantes ======
            contratantes_rows: List[tuple] = []
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                contratantes_rows.append((pessoa_id,))
            if contratantes_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
                        """INSERT INTO pessoas_contratantes
                           (id_pessoa, created_at, updated_at)
                           VALUES %s ON CONFLICT DO NOTHING""",
                        contratantes_rows,
                        template="(%s, NOW(), NOW())"
                    )

            # ====== NOVO v3.1: INSERT INTO pessoas_tipos (contratante=6) ======
            tipos_rows = [(pid, cfg.TIPO_PESSOA_CONTRATANTE_ID) for pid in new_ids if pid]
            if tipos_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
            ...
            for i, row in enumerate(chunk):
                ...
                if atividade:
                    renda = safe_float(row.get("renda"))
                    prof_rows.append((pessoa_id, atividade[:100], renda))
            if prof_rows:
                self._insert_profissoes_batch(prof_rows)
...
class Phase12ImovelProprietario(BasePhase):
    ...
    def run(self) -> int:
        ...
        count = 0
        # locimovelprop: imovel, proprietario, percentual, datahora, id

        # Collect ALL candidates per imovel (not last-write-wins)
        imovel_candidates: Dict[int, list] = {}  # imovel_new_id -> [(percentual, row_id, prop_new_id), ...]
        skipped = 0
        for row in self.parser.iter_table("locimovelprop"):
            imovel_old = safe_int(row.get("imovel"))
            prop_old = safe_int(row.get("proprietario"))
            percentual = safe_float(row.get("percentual") or 100.0)
            row_id = safe_int(row.get("id") or 0)
            ...
            if imovel_new_id not in imovel_candidates:
                imovel_candidates[imovel_new_id] = []
            imovel_candidates[imovel_new_id].append((percentual, row_id, prop_new_id))

        # Deterministic selection: highest percentual, then latest row_id
        updates: Dict[int, int] = {}
        multi_owner_count = 0
        for imovel_new_id, candidates in imovel_candidates.items():
            if len(candidates) == 1:
                updates[imovel_new_id] = candidates[0][2]
            else:
                # Sort by percentual DESC, then row_id DESC (tie-break: latest record)
                sorted_cands = sorted(candidates, key=lambda x: (x[0], x[1]), reverse=True)
                principal = sorted_cands[0][2]
                updates[imovel_new_id] = principal
                # Log co-owners in observacoes
                co_owners = [c[2] for c in sorted_cands[1:]]
                co_note = f"[Migração] Co-proprietários (pessoa IDs): {', '.join(str(c) for c in co_owners)}"
                try:
                    self.writer.execute(
                        "UPDATE imoveis SET observacoes = COALESCE(observacoes || E'\\n', '') || %s WHERE id = %s",
                        (co_note, imovel_new_id)
                    )
                except Exception as e:
                    log.warning(f"Erro ao gravar co-proprietarios imovel {imovel_new_id}: {e}")
                multi_owner_count += 1

        log.info(f"locimovelprop: {len(updates)} imoveis a atualizar ({multi_owner_count} com multi-proprietarios, {skipped} pulados).")
        ...
        # v3.1: Atualizar enderecos.id_pessoa dos imóveis para o proprietário correto
        # (Fase 11 insere com placeholder; agora corrigimos para o dono real)
        log.info("Fase 12: atualizando enderecos de imoveis para o proprietario correto...")
        self.writer.execute(
            """UPDATE enderecos e
               SET id_pessoa = i.id_pessoa_proprietario
               FROM imoveis i
               WHERE i.id_endereco = e.id
                 AND i.id_pessoa_proprietario IS NOT NULL
                 AND e.id_pessoa != i.id_pessoa_proprietario"""
        )
        self.writer.commit()
        log.info("Fase 12: enderecos de imoveis atualizados para proprietario correto.")

        self.state.save()
        log.info(f"Fase 12 concluida: {count} vinculos proprietario atualizados.")
        return count
...
class Phase13Inquilinos(BasePhase):
    ...
    def run(self) -> int:
        ...
        # Pre-load fiador-inquilino mapping from locfiador_inq (Correction 2)
        inq_to_fiador_old: Dict[int, int] = {}
        for lfi_row in self.parser.iter_table("locfiador_inq"):
            inq_old_id = safe_int(lfi_row.get("codinq"))
            fiad_old_id = safe_int(lfi_row.get("codfiador"))
            if inq_old_id and fiad_old_id:
                inq_to_fiador_old[inq_old_id] = fiad_old_id
        log.info(f"locfiador_inq: {len(inq_to_fiador_old)} vinculos inquilino-fiador carregados.")
        ...
        for chunk_start in range(0, len(pending), self.CHUNK_SIZE):
            ...
            for row in chunk:
                old_id = safe_int(row.get("codigo"))
                nome = safe_str(row.get("nome")) or "SEM NOME"
                nascto = safe_date(row.get("nascto")) or date(1900, 1, 1)
                ...
                pai = safe_str(row.get("pai")) or ""
                mae = safe_str(row.get("mae")) or ""
                nacion = safe_str(row.get("nacionalidade")) or ""
                nacion_id = cfg.DEFAULT_NACIONALIDADE_ID if nacion.lower().startswith("brasil") else None
                cpf = safe_str(row.get("cpf")) or ""
                rg = safe_str(row.get("rg")) or ""
                ...
                pessoa_id = self.resolve_or_create_pessoa(
                    ...
                    nome=nome[:100],
                    data_nascimento=nascto,
                    tipo_pessoa_id=cfg.TIPO_PESSOA_INQUILINO_ID,
                    tipo_pessoa_str=cfg.TIPO_PESSOA_INQUILINO_ID,
                    fisica_juridica=fisica_juridica,
                    estado_civil_id=estado_civil_id,
                    renda=None,
                    nome_pai=pai[:100] if pai else None,
            ...
            if not any(new_ids):
                log.error(f"Fase 13: resolve_or_create retornou 0 pessoas para {len(chunk)} inquilinos")
                self.writer.rollback()
                continue
            ...
            for i, row in enumerate(chunk):
                ...
                for tel_field in ("tel", "celular", "telcom", "fax", "outros"):
                    tel = re.sub(r"\s+", "", safe_str(row.get(tel_field)) or "")[:25]
                    if tel:
                        tipo_tel = cfg.TELEFONE_FIELD_MAP.get(tel_field, cfg.TIPO_TELEFONE_CELULAR_ID)
                        tels_data.append((pessoa_id, tel, tipo_tel))
                ...
                # Correction 2: Fiador via locfiador_inq (NOT row.get("fiador"))
                fiador_old_id = inq_to_fiador_old.get(old_id)
                fiador_new_id = self.state.get_new_id("locfiador", fiador_old_id) if fiador_old_id else None
                ...
                if imovel_new_id and ini_contrato:
                    ...
                    # Correction 2: Deterministic tipo_garantia
                    caucao_val = safe_float(row.get("valorcaucao") or 0)
                    if fiador_new_id:
                        tipo_garantia = "fiador"
                    elif safe_int(row.get("caucao")):
            ...
            # ====== NOVO v3.1: INSERT INTO pessoas_tipos (inquilino=12) ======
            tipos_rows = [(pid, cfg.TIPO_PESSOA_INQUILINO_ID) for pid in new_ids if pid]
            if tipos_rows:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        cur,
            ...
            for i, row in enumerate(chunk):
                ...
                profissao_str = safe_str(row.get("profissao"))
                if profissao_str:
                    prof_rows.append((pessoa_id, profissao_str[:100], 0.0))
            if prof_rows:
                self._insert_profissoes_batch(prof_rows)

            # ====== NOVO v3.2: Endereços dos inquilinos ======
            # Prioridade 1: endereço próprio (campos endac/complac/bairroac/cidadeac/estac/cepac)
            # Prioridade 2: se não tem endereço próprio, copiar do imóvel locado
            addr_data_inq: List[dict] = []
            imovel_copy_inq: List[tuple] = []  # (pessoa_id, imovel_old_id)

            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                endereco_str = safe_str(row.get("endac")) or ""
                if endereco_str:
                    # Tem endereço próprio
                    addr_data_inq.append({
                        "pessoa_id": pessoa_id,
                        "endereco": endereco_str,
                        "numero": "",
                        "complemento": safe_str(row.get("complac")) or "",
                        "bairro": safe_str(row.get("bairroac")) or "",
                        "cidade": safe_str(row.get("cidadeac")) or "",
                        "estado": safe_str(row.get("estac")) or "SP",
                        "cep": safe_str(row.get("cepac")) or "",
                    })
                else:
                    # Sem endereço próprio — copiar do imóvel locado
                    imovel_old = safe_int(row.get("imovel"))
                    if imovel_old:
                        imovel_copy_inq.append((pessoa_id, imovel_old))

            # Inserir endereços próprios
            for addr in addr_data_inq:
                try:
                    logr_id = self._get_or_create_logradouro(
                        addr["endereco"], addr["numero"], addr["bairro"],
                        addr["cidade"], addr["estado"], addr["cep"]
                    ...
                    log.warning(f"Endereco inquilino pessoa {addr['pessoa_id']}: {e}")

            # Copiar endereço do imóvel para inquilinos sem endereço próprio
            # Tier 2: from mapped imovel in new DB
            # Tier 3: from dump directly if imovel not mapped
            dump_addr_needed: List[tuple] = []  # (pessoa_id, imovel_old_id) for tier 3

            for pessoa_id, imovel_old in imovel_copy_inq:
                try:
                    imovel_new_id = self.state.get_new_id("locimovel", imovel_old)
                    if not imovel_new_id:
                        dump_addr_needed.append((pessoa_id, imovel_old))
                        continue
                    end_row = self.writer.fetch_all(
                        """SELECT e.id_logradouro, e.id_tipo, e.end_numero, e.complemento
                           FROM enderecos e
                           JOIN imoveis i ON i.id_endereco = e.id
                           WHERE i.id = %s""",
                        (imovel_new_id,)
                    )
                    if end_row:
                        logr_id, tipo_id, numero, complemento = end_row[0]
                        self._insert_endereco(pessoa_id, logr_id, str(numero) if numero is not None else "0", str(complemento) if complemento else None)
                    else:
                        dump_addr_needed.append((pessoa_id, imovel_old))
                except Exception as e:
                    log.warning(f"Endereco imovel->inquilino pessoa {pessoa_id}: {e}")

            # Tier 3: Read address directly from dump for unmapped imóveis
            if dump_addr_needed:
                needed_imovel_ids = {x[1] for x in dump_addr_needed}
                imovel_dump_cache: Dict[int, dict] = {}
                for im_row in self.parser.iter_table("locimoveis"):
                    im_old = safe_int(im_row.get("codigo"))
                    if im_old in needed_imovel_ids:
                        imovel_dump_cache[im_old] = dict(im_row)
                    if len(imovel_dump_cache) == len(needed_imovel_ids):
                        break  # all found

                for pessoa_id, imovel_old in dump_addr_needed:
                    im_data = imovel_dump_cache.get(imovel_old)
                    if not im_data:
                        log.warning(f"Tier3: imovel {imovel_old} nao encontrado no dump para pessoa {pessoa_id}")
                        continue
                    try:
                        endereco_str = safe_str(im_data.get("endereco")) or ""
                        if not endereco_str:
                            continue
                        logr_id = self._get_or_create_logradouro(
                            endereco_str, "",
                            safe_str(im_data.get("bairro")) or "",
                            safe_str(im_data.get("cidade")) or "",
                            safe_str(im_data.get("estado")) or "SP",
                            safe_str(im_data.get("cep")) or "",
                        )
                        if logr_id:
                            self._insert_endereco(pessoa_id, logr_id, "0",
                                safe_str(im_data.get("complemento")) or None)
                            log.debug(f"Tier3 endereco: pessoa {pessoa_id} <- imovel {imovel_old} (dump)")
                    except Exception as e:
                        log.warning(f"Tier3 endereco falhou pessoa {pessoa_id}, imovel {imovel_old}: {e}")

            # ====== v3.2: CÔNJUGES de inquilinos (locinquilino.nomecjg) ======
            for i, row in enumerate(chunk):
                pessoa_id = new_ids[i] if i < len(new_ids) else None
                if not pessoa_id:
                    continue
                ...
                if not nomecjg:
                    continue
                try:
                    conj_id = self._insert_pessoa(
                        nomecjg, cfg.TIPO_PESSOA_INQUILINO_ID, "fisica", True,
                        None, None, None, None, None,
                        f"Cônjuge do inquilino pessoa #{pessoa_id}",
                    )
                    if not conj_id:
                        continue
                    # pessoas_tipos
                    self.writer.execute(
                        "INSERT INTO pessoas_tipos (id_pessoa, id_tipo_pessoa, data_inicio, ativo) VALUES (%s, %s, CURRENT_DATE, TRUE) ON CONFLICT DO NOTHING",
                        (conj_id, cfg.TIPO_PESSOA_INQUILINO_ID)
                    )
                    # Atualizar observacoes do inquilino com referência ao cônjuge
                    existing_obs = self.writer.fetch_one(
                        "SELECT observacoes FROM pessoas WHERE idpessoa = %s", (pessoa_id,)
                    )
                    obs_current = existing_obs[0] if existing_obs and existing_obs[0] else ""
                    new_obs = f"{obs_current}\nCônjuge: {nomecjg} (pessoa #{conj_id})".strip()
                    self.writer.execute(
                        "UPDATE pessoas SET observacoes = %s WHERE idpessoa = %s",
                        (new_obs, pessoa_id)
                    )
                    log.debug(f"Cônjuge inquilino '{nomecjg}' -> pessoa {conj_id} (inquilino {pessoa_id})")
                except Exception as e:
                    log.warning(f"Erro cônjuge inquilino pessoa {pessoa_id}: {e}")
...
class Phase14Recibos(BasePhase):
    ...
    def run(self) -> int:
        ...
        # Pre-load imovel -> proprietario mapping
        imovel_to_prop: Dict[int, int] = {}
        for row in self.writer.fetch_all("SELECT id, id_pessoa_proprietario FROM imoveis WHERE id_pessoa_proprietario IS NOT NULL"):
            imovel_to_prop[row[0]] = row[1]
            ...
            try:
                with self.writer.conn.cursor() as cur:
                    result = psycopg2.extras.execute_values(
                        ...
                        sql_insert,
                        rows,
                        template="(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'aluguel','migracao_mysql',%s,TRUE,NOW(),NOW())",
                        fetch=True,
                    )
                    new_ids = [r[0] for r in result]
        ...
        for row in self.parser.iter_table("locrecibo"):
            ...
            proprietario_new_id = imovel_to_prop.get(imovel_new_id) if imovel_new_id else None
...
class Phase15RecibosHistorico(BasePhase):
    ...
    def run(self) -> int:
        ...
        # Step 3: Para recibos sem conta 1001, o valor_principal ficou = dump.valor (total)
        # enquanto as verbas foram setadas separadamente, duplicando.
        # Corrigir: valor_principal = valor_principal - soma_verbas_separadas
        # onde o recibo NÃO teve conta 1001 (valor_principal não foi substituído pela Phase15)
        ids_com_1001 = {lid for lid, verbas in updates.items() if "valor_principal" in verbas}
        ids_sem_1001 = {lid for lid in updates.keys() if lid not in ids_com_1001}
        if ids_sem_1001:
            log.info(f"Corrigindo valor_principal para {len(ids_sem_1001)} recibos sem conta 1001...")
            fix_rows = [(lid,) for lid in ids_sem_1001]
            for i in range(0, len(fix_rows), self.CHUNK_SIZE):
                chunk = fix_rows[i:i + self.CHUNK_SIZE]
                try:
                    with self.writer.conn.cursor() as cur:
                        psycopg2.extras.execute_values(
                            cur,
                            """UPDATE lancamentos_financeiros lf
                               SET valor_principal = lf.valor_principal
                    ...
                    log.error(f"Fix valor_principal sem conta 1001: {e}")
                    self.writer.rollback()

        # Step 4: Recalcular valor_total = soma_verbas + multa + juros
        log.info("Recalculando valor_total para todos os recibos migrados...")
        self.writer.execute("""
            UPDATE lancamentos_financeiros
            SET valor_total = COALESCE(valor_principal, 0) + COALESCE(valor_condominio, 0)
                + COALESCE(valor_iptu, 0) + COALESCE(valor_agua, 0) + COALESCE(valor_luz, 0)
                + COALESCE(valor_gas, 0) + COALESCE(valor_outros, 0)
                + COALESCE(valor_multa, 0) + COALESCE(valor_juros, 0)
            WHERE origem = 'migracao_mysql'
        """)
        self.writer.commit()
...
class Phase16LancamentoCC(BasePhase):
    ...
    def run(self) -> int:
        ...
        sql_insert = """
            INSERT INTO lancamentos_financeiros (
                id_imovel, id_inquilino, id_proprietario, id_conta_bancaria,
            ...
            try:
                with self.writer.conn.cursor() as cur:
                    psycopg2.extras.execute_values(
                        ...
                        sql_insert,
                        rows,
                        template="(%s,%s,%s,%s,%s,%s,%s,%s,%s,'pago',%s,'extrato_cc_migracao',%s,TRUE,NOW(),NOW())"
                    )
                self.writer.commit()
                return len(rows)
            except Exception as e:
...
class Phase18Repasses(BasePhase):
    ...
    def run(self) -> int:
        ...
        for row in self.parser.iter_table("locrepasse"):
            ...
            imovel_new_id = self.state.get_new_id("locimovel", imovel_old) if imovel_old else None
            if imovel_new_id == 0:
                imovel_new_id = None
            # Se não tem imóvel no dump, inferir se proprietário tem apenas 1 imóvel
            if not imovel_new_id and locador_new_id:
                prop_imoveis = self.writer.fetch_all(
                    "SELECT id FROM imoveis WHERE id_pessoa_proprietario = %s", (locador_new_id,)
                )
                if len(prop_imoveis) == 1:
                    imovel_new_id = prop_imoveis[0][0]
...
class MigrationOrchestrator:
    ...
    def run_phase(self, phase_id: str) -> int:
        ...
        try:
            ...
            # Handle both legacy int-returning phases and new PhaseStats-returning phases
            if isinstance(result, PhaseStats):
                stats = result
                count = stats.inseridos + stats.atualizados
                stats.log_summary(phase_id)

                if stats.has_critical_errors():
                    log.error(
                        f"Fase {phase_id}: {stats.erros} erros criticos. "
                        f"NAO marcando como concluida."
                    )
                    return 0  # Do NOT mark as done

                log.info(f"Fase {phase_id} OK: {count} registros em {elapsed:.1f}s")
                self.state.mark_phase_done(phase_id)
                return count
            else:
                # Legacy int return (phases not yet converted)
                count = result
                log.info(f"Fase {phase_id} OK: {count} registros em {elapsed:.1f}s")
                self.state.mark_phase_done(phase_id)
                return count
