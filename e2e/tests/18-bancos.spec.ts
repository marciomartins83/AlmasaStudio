...
test.describe.serial('Bancos CRUD', () => {
  let bancoId: string;
  const testData = {
    nome: `Test Banco E2E ${Date.now()}`,
    numero: `123456${Math.floor(Math.random() * 1000)}`
  };
  ...
  test('index page loads with table', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Bancos');
  ...
  test('new form page loads', async ({ page }) => {
    await page.goto('/banco/new');
    await waitForPageLoad(page);

    // Verify page loaded
    await expect(page).toHaveURL(/\/banco\/new/);

    // Verify form fields exist
    const nomeField = page.locator('input[name="banco[nome]"]');
    const numeroField = page.locator('input[name="banco[numero]"]');
  ...
  test('table has action buttons (edit/delete)', async ({ page }) => {
    ...
    const noRecordsMessage = page.locator('table tbody tr td:has-text("Nenhum banco cadastrado")');
  ...
  test('create new banco', async ({ page }) => {
    await page.goto('/banco/new');
    await waitForPageLoad(page);

    // Fill form
    await page.fill('input[name="banco[nome]"]', testData.nome);
    await page.fill('input[name="banco[numero]"]', testData.numero);

    // Submit form
    await page.click('button[type="submit"]');

    // Wait for navigation to complete
    await page.waitForURL(/\/banco/);
    await waitForPageLoad(page);
  ...
  test('created record appears in list', async ({ page }) => {
    ...
    // Look for the created banco in the table
    const row = page.locator('table tbody tr', {
      has: page.locator(`td:has-text("${testData.nome}")`)
    }).first();

    await expect(row).toBeVisible();
    await expect(row).toContainText(testData.numero);
  ...
  test('edit banco record', async ({ page }) => {
    // Navigate to edit page using the stored ID
    await page.goto(`/banco/${bancoId}/edit`);
    await waitForPageLoad(page);

    // Verify we're on the edit page
    await expect(page).toHaveURL(new RegExp(`/banco/${bancoId}/edit`));
  ...
  test('delete banco record', async ({ page }) => {
