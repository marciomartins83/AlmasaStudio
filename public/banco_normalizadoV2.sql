-- =====================================================
-- SCRIPT DE BANCO DE DADOS NORMALIZADO V2 - ALMASA STUDIO
...
1. Criação de tabelas de referência para normalização:
   - profissoes
   - estados_civis
   - nacionalidades
   - tipos_pessoa

2. Criação da tabela dados_pessoais para centralizar:
   - CPF, RG, data nascimento
   - Estado civil, profissão, naturalidade
   - Dados do cônjuge
   - Pai e mãe

3. Remoção de campos duplicados das tabelas:
   - inquilinos: removidos cpf, rg, estado_civil, profissao, pai, mae
   - fiadores: removidos cpf, rg, estado_civil, profissao, pai, mae
   - locadores: removidos cpf, rg, estado_civil, profissao, pai, mae

4. Normalização de dados bancários:
   - Uso da tabela contas_bancarias para relacionar pessoas com bancos
   - Remoção de campos de banco/agência duplicados

5. Correção de referências de usuário:
   - Campos usuario_baixa agora referenciam id do usuário do sistema
   - Campos de usuário em outras tabelas também corrigidos

6. Melhoria na estrutura de endereços:
   - Adicionado tipo_endereco para diferenciar residencial, comercial, etc.

7. Otimização de índices para melhor performance
*/ 